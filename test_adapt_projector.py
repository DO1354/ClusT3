...
def experiment(args):
    ...
    if proj_layer[0] != None:
        ...
        project_weights = checkpoint['projector']
        model.projector1.load_state_dict(project_weights)
    if proj_layer[1] != None:
        checkpoint = torch.load(args.root + 'weights/Projector2.pth')
        project_weights = checkpoint['projector']
        model.projector2.load_state_dict(project_weights)
    if proj_layer[2] != None:
        checkpoint = torch.load(args.root + 'weights/Projector3.pth')
        project_weights = checkpoint['projector']
        model.projector3.load_state_dict(project_weights)
    if proj_layer[3] != None:
        checkpoint = torch.load(args.root + 'weights/Projector4.pth')
        project_weights = checkpoint['projector']
        model.projector4.load_state_dict(project_weights)
    state = copy.deepcopy(model.state_dict())

    '''-------------------Optimizer----------------------------------'''
    if layers != [None, None, None, None]:
